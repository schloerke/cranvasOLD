...
glyph <- function(top = NULL, left = NULL, bottom = NULL, right = NULL, fill = "black", stroke = NA, size= 5,parent) {
  #error handling
...
draw.hbar <- function(mark,canvas) {
  ...
  if( !is.null(mark$bottom) ) {
    y <- c(mark$bottom + mark$parent$limits$top(), mark$bottom + mark$parent$limits$top())
  } else {
    y <- c( mark$parent$limits$height() - mark$top, mark$parent$limits$height() - mark$top )
    y <- y + mark$parent$limits$top() 
  }
  qlineWidth(canvas) <- mark$width
  qdrawLine(canvas, x, y, stroke= mark$stroke)
