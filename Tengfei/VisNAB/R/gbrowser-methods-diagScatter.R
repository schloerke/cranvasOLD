...
diagScatter <- function(pframe,xy=NULL){
  ...
  if(is.null(xy)){
    setPar(obj,'xy',names(pframe)[1:2])
  }else{
    setPar(obj,'xy',xy)
  }
  px <- pframe[[xy[1]]]
...
setMethod('gplot','diagScatter',function(obj,...){
  ...
  l.scatter<<-qlayer(s.scatter,
                   ...
                   mouseReleaseFun=releaseFun,
                    hoverMoveFun=hoverFun,
                     cache=TRUE)
  l.red<<-qlayer(s.scatter,paintFun=scatterFunRed,cache=TRUE)
##  l.zoom<<-qlayer(s.scatter,paintFun=scatterFunZoom,cache=TRUE)
...
scatterFun <- function(layer,painter){
  ...
  selectLogic <- !pframe$isSelected
  xy <- getPar(scatterObj,'xy')
  mar <- getPar(scatterObj,'mar')
  px <- pframe[[xy[1]]]
  py <- pframe[[xy[2]]]
  pxrange <- range(px)
  pxdiff <- diff(pxrange)
  ...
  if(sum(selectLogic)>0){
    qdrawCircle(painter,px[selectLogic],py[selectLogic],5,fill=pframe$fill[selectLogic],stroke=NA)}
  ## used for axis
  pxmin <- min(px)-0.1*pxdiff
  pxmax <- max(px)+0.1*pxdiff
  pymin <- min(py)-0.1*pydiff
...
##  l.zoom$setLimits(limits)
  ...
  ## xlab
  qdrawText(painter,xy[1],pxmax/2,pymin-pydiff*0.15,'center','top')
  ...
  ## ylab
  qdrawText(painter,xy[2],pxmin-0.15*pxdiff,pymax/2,'right','bottom',rot=90)
  selectLogic <- pframe$isSelected
  if(any(selectLogic)){
    qdrawCircle(painter,px[selectLogic],py[selectLogic],5,fill=pframe$fill[selectLogic],stroke=NA)
  }
  ##   selectLogic <- scatterObj@isHoverSelected
  ##   if(any(selectLogic)){
...
scatterFunRed <- function(layer,painter){
  ...
  pymin <- min(py)-0.1*pydiff
  pymax <- max(py)+0.1*pydiff
  selectLogic <- pframe$isHoverSelected
  if(any(selectLogic)){
    qdrawCircle(painter,px[selectLogic],py[selectLogic],5,fill='red',stroke=NA)
  }
}
...
pointIdentify <- function(layer,event){
  ...
  scatterObj@mutaframe$fill[hits] <<- 'blue'
  scatterObj@mutaframe$isSelected[hits] <<- TRUE
  ## scatterObj@mutaframe$isZoomedIn<<-rep(FALSE,nrow(pframe))
  ## scatterObj@mutaframe$isZoomedIn[hits] <<- TRUE
...
releaseFun <- function(layer,event){
  ...
  if(!(is.null(end.pos)|is.null(start.pos))){
    ...
    id <- which(px<bottomRight.x&px>topLeft.x&py<bottomRight.y&py>topLeft.y)
...
hoverFun <- function(layer,event){
  ...
  if(length(hits)>0){
    idx <- hits[1]
    ## althought I don't know why there could be more index found,..
    if(idx<=nrow(pframe)){
    scatterObj@mutaframe$isHoverSelected<<-rep(FALSE,nrow(scatterObj@mutaframe))
    scatterObj@mutaframe$isHighlighted<<-rep(FALSE,nrow(scatterObj@mutaframe))
    scatterObj@mutaframe$isHighlighted[idx]<<-TRUE
    scatterObj@mutaframe$isHoverSelected[idx]<<-TRUE
    qupdate(l.red)
    qupdate(layerIsland)
  }
  }else{
    scatterObj@mutaframe$isHoverSelected<<-rep(FALSE,nrow(pframe))
    qupdate(l.scatter)
  }
}
